...
class DeathService {
  ...
  /// Fetch all death records ordered by death date descending
  static Future<List<DeathRecordModel>> getDeathRecords() async {
    ...
    return (response as List)
        .map((row) => DeathRecordModel.fromMap(row as Map<String, dynamic>))
        .toList();
  }

  /// Validates and inserts a manual Death record into public.death_records
  static Future<DeathRecordModel> insertManualDeathRecord(Map<String, dynamic> data) async {
    ...
    // 3. Demographic & Chronological checks
    final ageError = SacramentalValidators.validateWholeNumberAge(data['age']?.toString());
    if (ageError != null) throw ageError;

    final dateOfDeath = DateTime.tryParse(data['date_of_death']?.toString() ?? '');
    final dateOfBurial = DateTime.tryParse(data['date_of_burial']?.toString() ?? '');
    final burialError = SacramentalValidators.validateBurialDate(dateOfBurial, dateOfDeath);
    if (burialError != null) throw burialError;
    ...
    if (duplicate != null) {
      throw 'This Book, Page, and Line reference is already registered in the Death Register.';
    }

    // 5. Generate unique record ID: DTH-YY-XXXX
